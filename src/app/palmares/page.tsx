    ...
    <div style={{ display: 'grid', gridTemplateColumns: compact ? '32px 1fr' : '44px 1fr auto', alignItems: 'center', gap: compact ? '10px' : '16px', padding: compact ? '14px 0' : '18px 0', borderBottom: '1px solid var(--border-light)' }}>
      ...
      <div>
        <div style={{ display: 'flex', alignItems: 'baseline', gap: '10px', flexWrap: 'wrap', marginBottom: '3px' }}>
          <div style={{ fontFamily: 'var(--font-barlow-condensed), sans-serif', fontWeight: 600, fontSize: compact ? '11px' : '14px', letterSpacing: '1px', textTransform: 'uppercase', color: 'var(--ink)' }}>{r.competition}</div>
          {r.date && <span style={{ fontFamily: 'var(--font-barlow-condensed), sans-serif', fontWeight: 500, fontSize: '10px', letterSpacing: '0.5px', color: 'var(--fire)' }}>📅 {r.date}</span>}
        </div>
        ...
        return (
          <div key={team} style={{ borderBottom: '1px solid var(--border-light)' }}>
            ...
            >
              ...
              <span style={{ display: 'block', color: isOpen ? 'var(--fire)' : 'var(--muted)', fontSize: '16px', transition: 'transform 0.2s', transform: isOpen ? 'rotate(90deg)' : 'rotate(0deg)' }}>→</span>
            </button>
